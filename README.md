...
Static artifacts package public ONNX Runtime headers at `onnxruntime/include/onnxruntime/*.h`. The package includes the public session headers, `cpu_provider_factory.h`, and enabled provider factory headers such as `coreml_provider_factory.h`, `nnapi_provider_factory.h`, `dml_provider_factory.h`, or `openvino_provider_factory.h`; raw upstream source directories such as `onnxruntime/include/onnxruntime/core` are rejected before upload.
...
The API 24 static contract restores the downstream Android `minSdk 24` behavior previously provided by shared-library delivery. The local downstream backup contained a shared ONNX Runtime `1.15.1` package, not `v1.22.0`, and the historical NDK/API flags for that package were not recoverable from this repository. The prior shared package did not expose static archive objects to the downstream JNI shared-library link, while the first static package was built above the Android ELF TLS floor and left API-29-only `__tls_get_addr` references to be resolved by consumers. Static Android builds now validate the `v1.22.0` static contract by pinning API 24, disabling LTO, forcing PIC, using `-femulated-tls` below API 29, and omitting the non-PIC Android `x86` MLAS AVX assembly path while keeping the `x86` ABI enabled through SSE/generic kernels.
