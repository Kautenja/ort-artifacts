...
2. **Determine the next spec number** (NO BRANCH CREATION):
   ...
   a. List existing root-level spec files:
      ```bash
      ls specs/[0-9][0-9][0-9]-*.md 2>/dev/null | sort
      ```

   b. Find the highest number N and use N+1 (zero-padded to 3 digits)

   c. Create the specs and checklist directories if needed:
      ```bash
      mkdir -p specs specs/checklists
      ```

   d. Set SPEC_FILE = `specs/NNN-short-name.md`
...
6. **Create a quality checklist** at `specs/checklists/NNN-short-name-requirements.md` using `templates/checklist-template.md`.
